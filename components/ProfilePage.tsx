...
const ProfilePage: React.FC<ProfilePageProps> = ({ userName, savedRoadmaps, onProgressToggle, onDeleteRoadmap, onUpdateRoadmap }) => {
    ...
    // This ensures that if the roadmap data changes (e.g., progress update), the view reflects it.
    useEffect(() => {
        if (selectedRoadmap) {
            const updatedRoadmap = savedRoadmaps.find(r => r.id === selectedRoadmap.id);
            setSelectedRoadmap(updatedRoadmap || null);
        }
    }, [savedRoadmaps, selectedRoadmap]);

    const handleSaveEdit = (updatedRoadmap: SavedRoadmap) => {
        onUpdateRoadmap(updatedRoadmap);
        setEditingRoadmap(null); // Exit edit mode, return to detailed view
    };
    
    // State 3: Editing a specific roadmap
    if (editingRoadmap) {
        return <RoadmapEditor roadmap={editingRoadmap} onSave={handleSaveEdit} onCancel={() => setEditingRoadmap(null)} />;
    }

    // State 2: Viewing details of a specific roadmap
    if (selectedRoadmap) {
        ...
        return (
            <div className="w-full max-w-5xl mx-auto py-8">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <button onClick={() => setEditingRoadmap(selectedRoadmap)} className="flex items-center px-4 py-2 rounded-md text-sm font-medium bg-slate-700 text-slate-300 hover:bg-slate-600 transition-colors">
    ...
    // State 1: List of all saved roadmaps
    return (
        <div className="w-full max-w-5xl mx-auto py-8">
                        ...
                        return (
                            <div key={roadmap.id} className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6 transition-all duration-300 group">
                                <div className="flex justify-between items-start">
                                    {/* Make the main body clickable to VIEW the roadmap */}
                                    ...
                                    <div className="flex items-center space-x-2 flex-shrink-0">
                                         {/* The EDIT button is now inside the detailed view */}
                                        <button onClick={() => onDeleteRoadmap(roadmap.id)} className="text-slate-500 hover:text-red-400 transition-colors p-1" aria-label="Delete roadmap">
